...
function generateServerComponent(astPayload, options = {}) {
  ...
  // Head machinery turns on when this component owns head markup, owns a
  // style block (styles hoist through the same head run), or when a child's
  // head/style must bubble through it. Gating on static presence keeps
  // head-free, component-free output byte-identical.
  const hasStyleMarkup = astPayload.style != null;
  const hasHeadMarkup = astPayload.head != null || hasStyleMarkup || hasComponentTags;
  ...
  if (hasHeadMarkup) {
    ...
    // Scoped-style dedup accumulator, threaded down the render tree through
    // options.styleScopes: the page render creates one array and every child
    // renderComponent receives the same instance, so a component imported by
    // several pages (or rendered several times on one page) injects its style
    // exactly once per document.
    builder.add(`const __wizzStyleScopes = (options && Array.isArray(options.styleScopes) && options.styleScopes) || [];`);
    builder.add('');
  }
  ...
  // Style emission is separate from emitHeadElement: the style tag carries no
  // data-wizz-head-id delivery tag — hydration verifies and claims head
  // slices positionally, and a runtime-deduplicated style cannot be part of a
  // compile-time expectation list. The client instead finds the delivered
  // style in the document head by its scope attribute and adopts it. CSS is
  // scoped at compile time and escaped for raw-text delivery: entities do not
  // work inside <style>, so `<` becomes the CSS escape `\3c ` (preventing a
  // premature `</style>` termination from string literals in the CSS).
  function emitStyleBlock(builder, style) {
    const at = `${style.loc?.start?.line ?? '?'}:${style.loc?.start?.column ?? '?'}`;
    const locValue = options.filePath ? `${options.filePath}:${at}` : at;
    const scopeLiteral = __wizzEscapeAttribute(style.scope);
    const cssLiteral = scopeCss(style.css, style.scope).replace(/</g, '\\3c ');
    builder.add(`if (__wizzStyleScopes.indexOf(${JSON.stringify(style.scope)}) === -1) {`).indent();
    builder.add(`__wizzStyleScopes.push(${JSON.stringify(style.scope)});`);
    builder.add(`__wizzHeadParts.push(${JSON.stringify(`<style data-wizz-style="${scopeLiteral}" data-wizz-loc="${__wizzEscapeAttribute(locValue)}">${cssLiteral}</style>`)});`);
    builder.dedent().add('}');
  }
  ...
  function emitComponentTag(builder, node) {
    ...
    // can attribute run nodes to the component that rendered them, and the
    // shared style-scope accumulator so styles inject once per document.
    const optionsArg = hasHeadMarkup
      ? `, { headOwner: __wizzHeadOwner + ${JSON.stringify(`/${node.componentId}`)}, styleScopes: __wizzStyleScopes }`
      : '';
    builder.add('{').indent();
  ...
  // declaration wins. Styles emit after the head block, gated by the shared
  // scope accumulator so exactly one style tag per component module reaches
  // the document.
  if (astPayload.head) {
    emitHeadBlock(builder, astPayload.head);
  }
  if (astPayload.style) {
    emitStyleBlock(builder, astPayload.style);
  }
