...
test('generated modules keep the output contract surface pinned to output 1.8.0', () => {
...
test('hydratable compiles exclude persistent vars from snapshot seeding', () => {
  const source = generateComponent(assignNodeIds(analyzeDependencies(parseComponent(
  ...
  assert.doesNotMatch(source, /hasOwnProperty\.call\(state, "theme"\)/);
});
