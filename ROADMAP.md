...
- Add `<wizz:head>` as a special block in the parser and analyzer (colon spelling to match the existing `on:` directive convention): a `HeadBlock` node, not a literal element, so it never renders into body markup. Restrict children to `title`, `meta`, and `link` with located, file-aware diagnostics for anything else (`style` is deliberately deferred to the styles milestone; reactive head updates are deliberately out of scope — head follows navigation, not state changes).
...
## 16. Component Stylesheets

**Goal:** Make `<style>` a first-class part of `.wizz` files: components own their CSS, it is delivered without a flash of unstyled content, never duplicated across pages, and extracted for production builds. Global (unscoped) styles are the milestone's core; Svelte-style scoped styles are a deliberate follow-up, not a day-one requirement.

- Teach the tokenizer that `<style>` is a raw-text element (parallel to the existing `<script>` raw-text mode): CSS braces, colons, and quotes must never reach the template expression lexer — today `p { color: red; }` throws `Template Expression Error` because braces parse as template expressions.
- Extract style content per component as opaque author text. Styles are trusted author input like scripts: expression output is not interpolated into CSS (values stay static), which sidesteps the injection class entirely. Both the server and client targets treat style blocks as non-body content — never rendered into `#app` markup.
- Deliver styles through the milestone 15 head machinery: the development server and hydration path hoist one deduped `<style>` per component module (deduped by module, not instance — a component imported by several pages must inject exactly once), and `wizz build` extracts all component styles into `dist/app.css` with a link tag in the copied document shell.
- Add scoped styles as a follow-up bullet once global styles ship: a minimal zero-dependency CSS selector parser, a scoping attribute stamped through the analyzer's existing attribute machinery, terminal-element selector rewriting per compound selector, a `:global(...)` escape hatch, and keyframes/`@media` block awareness. Scoping changes rendered markup whenever a component has styles, so it requires its own output-version bump assessment.
- Test end to end: raw-text parsing (braces, quotes, `@media` blocks), single injection across pages sharing a styled component, server-delivered styles with no duplicate after hydration, `wizz build` extraction, and failure states (unclosed `<style>`, style blocks inside `{#each}` bodies).
- Assess version impact before implementation: raw-text `<style>` acceptance is a syntax change; extraction is build-level; any generator changes (attribute stamping) follow the output-version contract. Record the bump in the CHANGELOG.

**Done when:** a component containing `<style>` renders with its CSS applied from the first `wizz dev` load (server-delivered in the head, adopted on hydration without duplicates, never present in `#app` body markup), and `wizz build` produces `dist/app.css` linked from the shell.
