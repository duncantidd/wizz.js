...
│   ├── dev.js                            Builds, serves dist (server-rendering eligible routes), runs /api server handlers, and watches .wizz source files
│   ├── apiRoutes.js                      Server API route surface: discovery, .env.server loading, request context, response shaping, guards
...
│   ├── apiRoutes.test.js                 Unit tests for route discovery, env loading, request/response shaping, and the guards
